  ...
  Comment.findAll({
    ...
    .then((comments) => {
      if (comments.length < 0)
        return res.status(404).json({ error: "Aucuns posts trouvés !" });
    ...
    .then((user) => {
      ...
      Comment.create(commentObject)
        .then(() =>
          res
            .status(201)
            .json({ message: "Le nouveau commentaire a été enregistré !" })
        )
  ...
  Comment.findOne({ where: { id: req.params.id } })
    .then((comment) => {
      Comment.update(
        ...
        .catch((error) =>
          res.status(400).json({
            message: "voici l'erreur lors de l'enregistrement: " + error,
          })
        );
    })
