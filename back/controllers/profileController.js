...
exports.getOneUser = (req, res, next) => {
    ...
    }).then(user => res.status(200).json(user)).catch(error => res.status(500).json(error.message))
}
            ...
            bcrypt.hash(req.body.password, 10)
                .then(hash => {
                    User.update({ password: hash }, { where: { id : req.params.userId } })
                        .then(() => res.status(200).json({ message: 'Mot de passe de l\'utilisateur mis à jour !' }))
    ...
    User.findOne({ where: { id: req.params.userId } })
        .then(user => {
            ...
            if (req.file) {
                ...
                User.update({ imageUrl: imageUrl }, { where: { id : req.params.userId } })
                    .then(() => res.status(200).json({ message: 'Utilisateur mis à jour !' }))
                    .catch(error => res.status(400).json({ error }));
            }
        })
...
exports.editUserAvatar = (req, res, next) => {
    ...
    User.findOne({ where: { id: req.params.userId } })
        .then(user => {
            ...
            // on contrôle s'il y a une nouvelle image
            if (req.file) {
                const avatar = `${req.protocol}://${req.get('host')}/images/avatars/${req.file.filename}`
                ...
                User.update({ avatar: avatar }, { where: { id : req.params.userId } })
                    .then(() => res.status(200).json({ message: 'Utilisateur mis à jour !', filename: avatar }))
                    .catch(error => res.status(400).json({ error }));
            }
        })
        .catch(error => res.status(500).json({ error }));
    //const {avatar } = req
    //console.log(avatar)
    //if(avatar.detectedFileExtension !== ".jpg") next(new Error("Invalid file type"))
...
exports.deleteUser = (req, res, next) => {
    ...
    User.findOne({ where: { id: req.params.userId } })
        .then(user => {
            ...
            //console.log(user.avatarUrl.split('/avatars/')[1])
