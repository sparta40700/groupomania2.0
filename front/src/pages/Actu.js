        ...
        .then(async (posts) =>{
          ...
          //setAllPosts(posts.data)
            for(const [index, post] of Object.entries(posts.data)){
                //console.log(post.id)
                await commentService.getNumbersOfCommentsForPost(currentUser.token, post.id)
                    ...
                    <div className="card" style={{ width: '18rem' }}>
                      ...
                      <div className="card-body">
                          {post.numberOfComments > 0 ? (
                              <span className="badge bg-info text-dark ms-3">{post.numberOfComments} commentaires</span>
                          ) : (
                              <span className="badge bg-info text-dark ms-3">Aucuns commentaires</span>
                          )}

                          {post.nbLikes > 0 ? (
                              <span className="badge bg-primary text-white ms-3">{post.nbLikes} likes</span>
                          ) : (
                              <span className="badge bg-primary text-white ms-3">Aucuns likes</span>
                          )}

                          {post.nbDislikes > 0 ? (
                              <span className="badge bg-danger text-white ms-3">{post.nbDislikes} dislikes</span>
                          ) : (
                              <span className="badge bg-danger text-white ms-3">Aucuns dislikes</span>
                          )}
                        ...
                        <Link to={`/onePost/${post.id}`}>
                          <button className="btn btn-primary">Lire le post complet</button>
                        </Link>
                        {(currentUser.userId === post.userId || admin === true) && (
                            <div>
                                <Link to={`/modifyPost/${post.id}`}>
                                    <button className="btn btn-primary mt-3">Modifier le post</button>
                                </Link>
                                <button className="btn btn-danger mt-3" onClick={() => deletePost(post.id)}>Supprimer le post</button>
                            </div>
                        )}
                      </div>
                    </div>
